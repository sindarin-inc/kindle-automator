...
class RequestBodyLogger:
    ...
    def sanitize_sensitive_data(data):
        ...
        sensitive_keys = ['password', 'email', 'token', 'secret', 'key', 'auth', 
                          'credential', 'access_token', 'refresh_token', 'code']
        ...
        for key in sanitized:
            ...
            # And sanitize lists of dictionaries
                ...
                sanitized[key] = [RequestBodyLogger.sanitize_sensitive_data(item) if isinstance(item, dict) else item 
    ...
    def log_request():
        ...
        if request_data:
            if isinstance(request_data, (dict, list)):
                ...
                if len(json_str) > 500:
                    logger.info(f"REQUEST [{request.method} {request.path}]: {json_str[:500]}... (truncated, total {len(json_str)} bytes)")
                else:
                    logger.info(f"REQUEST [{request.method} {request.path}]: {json_str}")
            elif isinstance(request_data, str) and len(request_data) > 500:
                logger.info(f"REQUEST [{request.method} {request.path}]: {request_data[:500]}... (truncated, total {len(request_data)} bytes)")
            else:
                logger.info(f"REQUEST [{request.method} {request.path}]: {request_data}")
        else:
            logger.info(f"REQUEST [{request.method} {request.path}]: No body")
    ...
    def log_response(response):
        ...
        if response.direct_passthrough:
            logger.info(f"RESPONSE [{request.method} {request.path}]: Direct passthrough (file/image)")
            return response
        ...
        try:
            ...
            try:
                ...
                if isinstance(response_data, dict):
                    ...
                    if len(json_str) > 500:
                        logger.info(f"RESPONSE [{request.method} {request.path}]: {json_str[:500]}... (truncated, total {len(json_str)} bytes)")
                    else:
                        logger.info(f"RESPONSE [{request.method} {request.path}]: {json_str}")
                else:
                ...
                if len(response_text) > 500:
                    logger.info(f"RESPONSE [{request.method} {request.path}]: {response_text[:500]}... (truncated, total {len(response_text)} bytes)")
                else:
                    logger.info(f"RESPONSE [{request.method} {request.path}]: {response_text}")
        except UnicodeDecodeError:
            logger.info(f"RESPONSE [{request.method} {request.path}]: Binary data ({len(original_data)} bytes)")
