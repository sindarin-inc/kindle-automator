...
class ZombieEmulatorCleaner:
    ...
    def is_avd_zombie(self, avd_name: str) -> bool:
        ...
        if is_zombie:
            logger.warning(f"AVD {avd_name} has zombie emulator: PID {proc['pid']}, "
    ...
    def clean_all_zombies_on_display(self, display_num: int) -> int:
        ...
        for line in output.splitlines():
            ...
            try:
                env_output = self._run_command(f"cat /proc/{pid}/environ 2>/dev/null | tr '\\0' '\\n' | grep DISPLAY", check=False)
        ...
        for pid in pids_to_check:
            ...
            try:
                ...
                if port_match:
                    ...
                    if not self._is_port_listening(port) and not self._is_emulator_in_adb(port):
                        logger.info(f"Found zombie emulator on display :{display_num} (PID: {pid}, port: {port})")
