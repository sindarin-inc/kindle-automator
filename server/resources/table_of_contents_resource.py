...
class TableOfContentsResource(Resource):
    ...
    @handle_automator_response
    def post(self):
        """Navigate to a specific chapter via POST request.

        JSON Body:
            chapter (str): Required - the chapter name to navigate to.
            sindarin_email (str): Required - email to identify which automator to use.

        Returns:
            JSON response with navigation result, OCR text, and position data.
        """
        server = AutomationServer.get_instance()
        ...
        if not chapter_name:
            return {"error": "Chapter parameter is required"}, 400
