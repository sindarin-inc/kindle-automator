...
class UserActivityResource(Resource):
    ...
    def _parse_user_activities(self, user_email):
        ...
        if os.path.exists(log_file):
            activities.extend(self._parse_log_file(log_file, user_email))

        # Check user-specific log file
        user_log_file = os.path.join(self.log_dir, f"{user_email}.log")
        if os.path.exists(user_log_file):
            activities.extend(self._parse_log_file(user_log_file, user_email))
                        ...
                        if match:
                            ...
                            if event_type == "open_book":
                                ...
                                if current_activity and current_activity.get("type") == "books_stream":
                                    duration = (timestamp - current_activity["timestamp"]).total_seconds()
