...
def get_connection_params(use_kindle_db=False):
    ...
    if is_docker_running():
        ...
        if not database_url:
            print("Error: DATABASE_URL not set and Docker container '{}' not running".format(DOCKER_CONTAINER))
            sys.exit(1)
...
def interactive_session():
    ...
    if is_docker_running():
        ...
        cmd = [
            ...
            "psql", "-p", LOCAL_DB_PORT, "-U", LOCAL_DB_USER, "-d", KINDLE_DB_NAME
        ]
        subprocess.run(cmd)
    else:
...
def dump_database(output_file):
    ...
    if is_docker_running():
        cmd = [
            ...
            "pg_dump", "-p", LOCAL_DB_PORT, "-U", LOCAL_DB_USER, 
            "-d", KINDLE_DB_NAME, "-n", KINDLE_SCHEMA
        ]
